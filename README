...
## Overview
FFT-based speech encryption and decryption system. Takes a speech signal, scrambles its frequency components using a secret key, and recovers it using the same key.
...
## Method
FFT-based Frequency-Domain Scrambling:
1. Apply FFT to speech signal
2. Permute frequency components using a secret key
3. Apply IFFT → encrypted audio
4. Reverse permutation with same key
5. Apply IFFT → recovered audio
...
| `decrypted_speech.wav` | Recovered audio |
| `results.png` | All 6 plots (waveforms + spectra) |
